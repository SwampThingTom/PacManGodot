...
@onready var _maze: TileMapLayer = $MazeMap
@onready var _pellets: PelletsMap = $PelletsMap
@onready var _fruit: BonusFruitActor = $BonusFruitActor
@onready var _player_one_text_renderer: TileMapLayer = $PlayerOneTextRenderer
@onready var _ready_text_renderer: TileMapLayer = $ReadyTextRenderer
@onready var _game_over_text_renderer: TileMapLayer = $GameOverTextRenderer
@onready var _scores_renderer: ScoresRenderer = $ScoresRenderer
@onready var _status_renderer: StatusRenderer = $StatusRenderer
...
func _start_level() -> void:
    ...
    _status_renderer.update_fruits(_level)
    _pellets.reset_pellets()
    _pacman.on_start_level(_level)
    _ghost_mode.on_start_level(_level)
    _ghost_coordinator.on_start_level(_level)
    _fruit.on_start_level(_level)
...
func _start_round() -> void:
    ...
    _ghost_mode.on_start_round()
    _ghost_coordinator.on_start_round()
    _fruit.on_start_round()
...
func _player_died() -> void:
    ...
    _ghost_mode.on_player_died()
    _ghost_coordinator.on_player_died()
...
func _level_complete() -> void:
    ...
    _ghost_mode.on_level_complete()
    _ghost_coordinator.on_level_complete()
...
func _show_start_player_sequence() -> void:
    _player_one_text_renderer.show()
    _ready_text_renderer.show()
    await get_tree().create_timer(2.0).timeout
    _player_one_text_renderer.hide()
...
func _check_ghost_collisions() -> void:
    ...
    for ghost in _ghost_coordinator.get_ghosts():
...
func _reset_scores() -> void:
    ...
    _scores = [0, 0]
    _scores_renderer.clear_player_score(0)


func _spawn_actors() -> void:
    assert(_pacman == null, "Actors already spawned")
    var actor_factory := ActorFactory.new(_maze)

    _pacman = actor_factory.make_pacman(_pellets)
    add_child(_pacman)

    _targeting = GhostTargetingService.new(_pacman, _ghost_coordinator, false)
    _ghost_coordinator.on_start_game(
        actor_factory.make_blinky(_ghost_mode, _targeting), 
...
func _update_current_player_score(points: int) -> void:
    ...
    _scores_renderer.draw_player_score(_current_player, current_score)
    
    if current_score > _high_score:
        _high_score = current_score
        _scores_renderer.draw_high_score(current_score)
