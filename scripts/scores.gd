...
var digit_atlas_coords: Array[Vector2i] = [
    Vector2i(0, 2), # 0
    Vector2i(1, 2), # 1
    Vector2i(2, 2), # 2
    Vector2i(3, 2), # 3
...
func _clear_score_at(origin: Vector2i) -> void:
    ...
    set_cell(origin + Vector2i(4, 0), font_source_id, zero)
    set_cell(origin + Vector2i(5, 0), font_source_id, zero)
