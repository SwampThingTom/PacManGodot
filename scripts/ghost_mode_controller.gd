...
const MODE_DURATIONS: Array[Array] = [
    [7.0, 20.0, 7.0, 20.0, 5.0,   20.0, 5.0],    # Level 1
    [7.0, 20.0, 7.0, 20.0, 5.0, 1033.0, 0.016],  # Level 2-4
...
func _process(delta: float) -> void:
    ...
    if _mode_index >= MODE_DURATIONS[0].size():
        return
...
func on_start_level(level: int) -> void:
    _level = level
    _duration_index = _get_duration_index(_level)
...
func _get_duration() -> float:
    if _mode_index >= MODE_DURATIONS[0].size():
        return 0.0
    return MODE_DURATIONS[_duration_index][_mode_index]


# Used for accessing the MODE_DURATIONS array.
func _get_duration_index(level: int) -> int:
    if level == 1:
        return 0
    if level < 5:
        return 1
